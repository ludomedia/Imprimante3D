...
inline void process_commands()
{
  ...
  if(code_seen('M'))
  {
    ...
    switch( (int)code_value() ) 
    {
      ...
      case 105: // M105
        Serial.print("T:");
        // TESTING Serial.println( analog2temp(analogRead(TEMP_0_PIN)) ); 
        Serial.println("210.00"); // TESTING
...
inline void manage_inactivity(byte debug) { 
  // TESTING if( (millis()-previous_millis_cmd) >  max_inactive_time ) if(max_inactive_time) kill(debug); 
}
